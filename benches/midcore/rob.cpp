...
// all numbers are generated randomly at compile time. the internal state is
// pseudo remembered using the counter macro. the seed is based on time using
// the timestamp and time macro. additionally a custom random seed can be
// specified to fully rely
...
// generates a random number seeded with time and the custom seed between min
// and max ( [min, max[ )
#define DYC_RAND_NEXT_BETWEEN(min, max)                                        \
  (min + (::Dynlec::CTRandomGeneratorValueSeeded<__COUNTER__> % (max - min)))
// generates a random number seeded with time and the custom seed with a limit (
// [0, limit[ )
...
// x, y, z and c
template <uint64_t x, uint64_t y, uint64_t z, uint64_t c>
class CTRandomGeneratorRaw {
  static_assert(y != 0, "CompileTimeRandom can not be used with 'y' equals 0");
...
class CTRandomGeneratorRawSafe
    : public CTRandomGeneratorRaw<x, (y == 0) ? 1 : y,
                                  (z == 0 && c == 0) ? 1 : z, c> {};
...
template <uint64_t iterations, uint64_t seed> class CTRandomGenerator {
  friend CTRandomGenerator<iterations + 1, seed>;
  typedef
      typename CTRandomGenerator<iterations - 1, seed>::Current::Next Current;

public:
  constexpr static uint64_t Value = Current::Value;
};

template <uint64_t seed> class CTRandomGenerator<0ULL, seed> {
  friend CTRandomGenerator<1ULL, seed>;

  typedef typename CTRandomGeneratorRawSafe<
      seed ^ 1066149217761810ULL, seed ^ 362436362436362436ULL,
      seed ^ 1234567890987654321ULL, seed ^ 123456123456123456ULL>::Next
      Current;
...
constexpr static uint64_t CTRandomGeneratorValue =
    CTRandomGenerator<iteration, seed>::Value;

const uint64_t CTRandomSeed = (RAND_SEED);

template <uint64_t iteration>
constexpr static uint64_t CTRandomGeneratorValueSeeded =
    CTRandomGeneratorValue<iteration, CTRandomSeed>;
...
struct CTRandomStream {
  ...
  template <typename T> static void Call(T callback) {
    CTRandomStream<n - 1, seed>::Call(callback);
    callback(n - 1, CTRandomGeneratorValue<n, seed>);
  }
};

template <uint64_t seed> struct CTRandomStream<0, seed> {
  template <typename T> static void Call(T callback) {}
};
} // namespace Dynlec
...
int main() {
  ...
  auto array2 = new uint64_t[RSS_AS_KB];
  randomize(array2, RSS_AS_KB);

  const volatile uint64_t *p = &array[0];
  const volatile uint64_t *baseline = &array[0];
  const volatile uint64_t *p2 = &array2[8];
  const volatile uint64_t *baseline2 = &array2[0];

#endif
  for (int i = 0; i < ITERATIONS; i++) {
    p = RobFunroller<0>::generate(baseline, p);
    p2 = RobFunroller<0>::generate(baseline2, p2);
    p = RobFunroller<0>::generate(baseline, p);
    p2 = RobFunroller<0>::generate(baseline2, p2);
  }
}
